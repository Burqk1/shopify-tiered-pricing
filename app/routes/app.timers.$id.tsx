...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  return json({
    timer: {
      ...
      showOn: timer.showOn,
      linkedRuleId: timer.linkedRuleId || "",
      expiredMessage: timer.expiredMessage || "",
      hideOnExpiry: timer.hideOnExpiry,
      status: timer.status,
    },
...
export default function EditTimer() {
  const { timer, rules } = useLoaderData<typeof loader>();
  const submit = useSubmit();
  const navigation = useNavigation();

  const endDateTime = new Date(timer.endTime);
  const [name, setName] = useState(timer.name);
  const [endDate, setEndDate] = useState(endDateTime.toISOString().split("T")[0]);
  const [endTime, setEndTime] = useState(endDateTime.toISOString().split("T")[1].slice(0, 5));
  ...
  const [showOn, setShowOn] = useState<TimerShowOn>(timer.showOn as TimerShowOn);
  const [linkedRuleId, setLinkedRuleId] = useState(timer.linkedRuleId);
  const [expiredMessage, setExpiredMessage] = useState(timer.expiredMessage);
  const [hideOnExpiry, setHideOnExpiry] = useState(timer.hideOnExpiry);

  // Delete modal state
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);

  const isLoading = navigation.state === "submitting";
  const isExpired = new Date(timer.endTime) < new Date();
  ...
  return (
    ...
    >
      <BlockStack gap="500">
        {isExpired && (
          <Banner tone="warning">
            This timer has expired. Update the end time to reactivate it.
          </Banner>
        )}
