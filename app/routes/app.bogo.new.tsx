...
import { createBogoOffer } from "~/models/bogo.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    const offer = await createBogoOffer({
      shopId: shop.id,
      ...
      maxUsesPerOrder: data.maxUsesPerOrder ? parseInt(data.maxUsesPerOrder as string) : undefined,
      maxUsesTotal: data.maxUsesTotal ? parseInt(data.maxUsesTotal as string) : undefined,
    ...
    if (status === "ACTIVE") {
      const { updateBogoStatus } = await import("~/models/bogo.server");
      await updateBogoStatus(offer.id, "ACTIVE");
    }

    return redirect("/app/bogo");
  } catch (error) {
    return json({ error: "Failed to create BOGO offer" }, { status: 500 });
  }
};

export default function CreateBogo() {
  ...
  const [bogoType, setBogoType] = useState("BUY_X_GET_Y_FREE");
              ...
              <BlockStack gap="300">
                ...
                <BlockStack gap="200">
                  <Text variant="bodySm" as="p">
                    <strong>Buy X Get Y Free:</strong> Classic BOGO. Great for clearing inventory.
                  </Text>
                  <Text variant="bodySm" as="p">
                    <strong>Buy X Get Y % Off:</strong> "Buy 2, Get 3rd at 50% off" - higher perceived value.
                  </Text>
                  <Text variant="bodySm" as="p">
                    <strong>Spend X Get Y:</strong> Encourages larger cart sizes.
                  </Text>
                </BlockStack>
                <Divider />
                <Text variant="bodySm" tone="subdued" as="p">
                  BOGO offers are powerful sales drivers. The "free" item is typically the lowest-priced qualifying item.
                </Text>
              </BlockStack>
            </Card>
